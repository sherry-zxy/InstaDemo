 ...
 */
 ...
 /*
  * 
  * @param {*} success_cb:sucess callback
  * @param {*} error_cb:error callback
  * create_like: 回调函数，如果成功就调用定义success的那个函数，如果失败就是调用定义在error参数的函数
  */
function create_like(success_cb, error_cb) {
  /*
   * this 是找到的attribute, 找它的sibling然后找hidden-data再找post-pk，然后变成text显示出来
   * 找到当前点赞的是哪个post
   */
    ...
    /**
     * 调用ajax函数，发了一个request,访问url，传递post的pk
     */
    $.ajax({
        type: "POST",
        url: '/like',
        ...
        /**
         * 如果ajax成功了就success callback, fail就error callback
         */
...
/*
 * 
 * @param {*} data 这是ajax成功后的返回值有data-(result,post_pk)
 */
function like_update_view(data) {
    console.log(data);

    // toggle heart
    //找hidden data的post.pk 找要对哪个post进行操作
    var $hiddenData = $('.hidden-data.' + data.post_pk);
    if (data.result) { //如果result是1，代表点赞了，删除空心，添加实心的爱心
      $hiddenData.siblings('.submit-like').removeClass('fa-heart-o').addClass('fa-heart');
    } else {//如果result是0，代表点赞了，删除实心，添加空心
 ...
 /*
  *  用$加上attribute js会去html找这个attribute
  * xxx.on('click',function()) -> 监听
  * this是找到的那个tag i:                       
